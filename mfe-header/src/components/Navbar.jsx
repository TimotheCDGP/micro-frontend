...
function Navbar() {
  ...
  useEffect(() => {
    ...
    const handlePlayerJoined = (data) => {
      console.log('[EventBus] playerJoined reçu dans Navbar', data);
      setNotifications(prev => prev + 1); // incrémente le badge
    };

    eventBus.on('playerJoined', handlePlayerJoined);

    return () => {
      eventBus.off('playerJoined', handlePlayerJoined);
    };
  }, []);
